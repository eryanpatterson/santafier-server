...
async function verifyEmail(member, group, url) {
    const mailOptions = {
        from: 'emailelf@secretsantafier.com',
        to: member.email,
...
function santaEmail(group) {
    const { members, exchangeDate, spendLimit } = group;
    notifyAdmin(group);
    ...
    for (let i = members.length; i > 0; i--) {
        ...
        if (i === 1) {
            ...
            sendEmail(mailData, exchangeDate, spendLimit);
        } else {
            mailData = {
...
function notifyAdmin(group) {
    const mailOptions = {
        from: 'headelf@secretsantafier.com',
        to: group.adminEmail,
...
async function sendEmail(data, exchangeDate, spendingLimit) {
    ...
    if (data.address) {
        html = `Mail their gift to ${data.address} by ${exchangeDate}.`
    } else html = `Bring their gift to your in-person gift exchange on ${exchangeDate}.`;
    const mailOptions = {
        from: 'headelf@secretsantafier.com',
        to: data.email,
        subject: 'Secret Santa Result',
        html: `<h2>Congrats, ${data.name}!</h2> <p>You are ${data.giftee}'s Secret Santa!</p>
                <p>${html}. Be sure to stay within the ${spendingLimit} dollar spending limit!`
    };
