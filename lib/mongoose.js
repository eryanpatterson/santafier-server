...
async function group({ group, members }) {
    const nameInUse = await Group.findOne({"name": group.name});
    if (nameInUse) {
        throw new Error("Group name already in use; please choose another.");
    }
    ...
    newGroup.members.forEach( member => {
        const groupMember = {
            member: member,
            group: newGroup.name,
            useAddress: newGroup.useAddress
        }
        const token = jwt.sign(groupMember, process.env.VERIFY_TOKEN_SECRET);
...
async function verify(member) {
    const filter = { "members.name": member.member.name, "name": member.group};
    const update = {"$set": {
